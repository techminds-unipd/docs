...
- *CreateWorkflowService*
    - Proprietà:
        ...
        - #declaration("- getWorkflowPort: GetWorkflowPort") #arrow porta di output per il recupero di un _workflow_ dal database.
    - Operazioni:
        - #declaration("- createWorkflow(cmd: CreateWorkflowCommand): Workflow") #arrow controlla che il _workflow_ non esista già tramite la porta _getWorkflowPort_ (per approfondimento si veda DA METTERE RIFERIMENTO ALLA GET) , in caso contrario lancia un'eccezione di tipo _WorkflowAlreadyExistsError_. Successivamente, crea il _workflow_ vuoto e procede a salvarlo tramite la _createWorkflowPort_. Nel caso in cui il salvataggio non andasse a buon fine lancia un'eccezione di tipo _WorkflowNotAddedError_, altrimenti ritorna il _workflow_ aggiunto.
...
- *WorkflowPortAdapter*
    ...
    - Note:
        - sono state descritte le operazioni utilizzate unicamente da questa funzionalità;
