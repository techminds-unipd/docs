...
- *Decorator*: implementato da _NestJS_, il framework utilizzato per lo sviluppo del backend, per aggiungere metadati a classi, metodi e proprietà. I decorator vengono inoltre utilizzati per definire i vari componenti dell'applicazione, come i moduli e i controller, inclusi i vari endpoint delle API REST;
...
- *Dependency Injection*: viene implementato da _NestJS_ e permette di iniettare le dipendenze in modo automatico attraverso i decoratori _\@Injectable_ e _\@Inject_. La dipendenza viene iniettata nel costruttore della classe, in modo che il framework possa gestire la creazione dell'oggetto;
