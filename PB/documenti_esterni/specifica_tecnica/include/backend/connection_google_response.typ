...
- *ConnectionGoogleResponseController*
    ...
    - Operazioni:
        - #declaration("+ googleAuthCallback(code: string): TokenDTO") #arrow Chiama il metodo _getToken_ di _connectionGoogleResponseUseCase_ e converte il _Token_ ricevuto in un _TokenDTO_ per poi ritornalo. Nel caso in cui il _getToken_ non andasse a buon fine ritorna status http 500.
...
- *ConnectionGoogleRequestPort* (interfaccia)
    - #declaration("+ getAuthClientData(): Token").
