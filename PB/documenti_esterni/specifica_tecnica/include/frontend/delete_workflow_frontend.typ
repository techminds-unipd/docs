...
La classe contiene un unico metodo, #declaration("deleteWorkflowByName(name: string, accessToken: string): Promise<DeleteWorkflowResponse>"), che invia una richiesta DELETE all'endpoint API appropriato, utilizzando il nome del workflow come parametro nella URL.
