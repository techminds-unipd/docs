...
RegisterService è una classe dotata di un unico metodo asincrono #declaration("register(username: string, password: string): Promise<RegisterResponse>"). Tale metodo prende come parametri due stringhe, una per lo username e una per la password, e ritorna una Promise di tipo RegisterResponse. Quest'ultima è un'interfaccia che contiene username e password. Il metodo richiede di effetturare la registrazione all'endpoint API offerto dalla funzionalità RegisterUser del backend (vedi @register-user).
L'esecuzione prosegue in base al codice di risposta ricevuto:
- Se le credenziali sono corrette (codice HTTP 201) la Promise viene risolta e l'username e la password criptata ricevuti dal backend viene ritornato tramite LoginResponse;
