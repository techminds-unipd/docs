...
#figure(
  image("../../assets/frontend/CreateWorkflowService.svg", width: 50%),
...
#declaration[useCreateWorkflow(createWorkflowsService: CreateWorkflowService): IUseCreateWorkflow] è un hook React personalizzato che permette di creare un workflow utilizzando CreateWorkflowService, gestendo automaticamente lo stato del caricamento e gli eventuali errori. L'hook restituisce una IUseCreateWorkflow, ovvero un'interfaccia che contiene la funzione #declaration("createWorkflow(name:string): CreateWorkflowResponse[0..1]") per invocare la creazione di un nuovo workflow, i parametri isLoading ed error per la gestione della UI.
