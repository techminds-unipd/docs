...
#let getSprintData() = {
    ...
    lazzarin_consuntivo.push(  (6, "3 (+3)", 0, "4 (+4)", "2 (-3)", 0) )
    ...
    vallotto_consuntivo.push(  (0, "2 (+2)", "3 (+3)", "4 (+4)", "3 (+3)", "7 (-8)") )
    ...
    //Sprint 9
    bressan_preventivo.push(   (0, 4, 0, 3, 4, 8) )
    corradin_preventivo.push(  (0, 4, 2, 0, 5, 6) )
    lazzarin_preventivo.push(  (0, 0, 0, 5, 9, 4) )
    salviato_preventivo.push(  (0, 0, 0, 3, 10, 6) )
    squarzoni_preventivo.push( (0, 7, 0, 0, 3, 7) )
    tutino_preventivo.push(    (0, 0, 0, 5, 9, 4) )
    vallotto_preventivo.push(  (6, 3, 0, 3, 2, 2) )

    bressan_consuntivo.push(   (0, 4, 0, "2 (-1)", "5 (+1)", 8) )
    corradin_consuntivo.push(  (0, "2 (-2)", 2, 0, 5, "8 (+2)") )
    lazzarin_consuntivo.push(  (0, "2 (+2)", 0, "4 (-1)", "8 (-1)", 4) )
    salviato_consuntivo.push(  (0, 0, 0, "5 (+1)", "8 (-2)", 6) )
    squarzoni_consuntivo.push( (0, "9 (+2)", 0, 0, 3, "5 (-2)") )
    tutino_consuntivo.push(    (0, 0, 0, 5, "10 (+1)", "3 (-1)") )
    vallotto_consuntivo.push(  (6, "1 (-2)", 0, 3, "3 (+1)", "3 (+1)") )
...
#let getOreConsumatePersonaPerRuolo(sprintNumber: getSprintNumber() ) ={
    let (_, consuntivo) = getSprintData()
    let roleNumber = 6
...
//Ore totali per persona
    ...
    let oreTotaliPersonaRuolo = (
        ...
        lazzarin:(0,0,0,0,0,0), 
        salviato:(0,0,0,0,0,0), 
        squarzoni:(0,0,0,0,0,0), 
        tutino:(0,0,0,0,0,0), 
        vallotto:(0,0,0,0,0,0)
    )

    for i in range(0, sprintNumber) {
  
        for j in range(0,roleNumber){        
            oreTotaliPersonaRuolo.bressan.at(j) += getNumber(consuntivo.bressan.at(i).at(j))
            oreTotaliPersonaRuolo.corradin.at(j) += getNumber(consuntivo.corradin.at(i).at(j))
            oreTotaliPersonaRuolo.lazzarin.at(j) += getNumber(consuntivo.lazzarin.at(i).at(j))
            oreTotaliPersonaRuolo.salviato.at(j) += getNumber(consuntivo.salviato.at(i).at(j))
            oreTotaliPersonaRuolo.squarzoni.at(j) += getNumber(consuntivo.squarzoni.at(i).at(j))
            oreTotaliPersonaRuolo.tutino.at(j) +=   getNumber(consuntivo.tutino.at(i).at(j))
